...
def build_stub_sql(project_name: str, node: ManifestNode) -> str:
    # Versioned nodes: source table name is the alias (e.g. fct_booking_v1)
    # Unversioned nodes: source table name is the model name
    source_table = node.alias or node.name if node.version is not None else node.name
    return (
