...
└── jaffle_finance/
    ...
    └── models/
        ...
        └── integrated/
            ├── sources.yml
            ├── schema.yml
            ├── fct_booking.sql        # unversioned alias → latest version's physical table
            ├── fct_booking_v1.sql
            └── fct_booking_v2.sql
```
...
Source `name` is the upstream project name. Table `name` and `identifier` are both set to `node["alias"]`. Database, schema, and alias are resolved directly from the manifest — no derivation.
...
The package is environment-specific by design. To target a different environment, either re-run `inject` against that environment's `manifest.json` re-run `inject` against that environment's `manifest.json`.
...
Unversioned alias stub points at the same physical table as the latest version:

```sql
-- Generated by dbt-package-loom. Do not edit.
-- Alias for latest version (v2)
select * from {{ source('jaffle_finance', 'fct_booking_v2') }}
```

Latest version is determined by `latest_version` in the manifest node, or max version number if absent.
...
This ensures `ref('jaffle_finance', 'fct_booking', v=1)` resolves correctly.
...
| `alias` | `node.alias` | Physical table/view name; used as table `name` and `identifier` in `sources.yml` and as the SQL filename stem |
